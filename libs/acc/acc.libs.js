...
const GetUserbyUserId = async (userId, projectId, token) => {
    ...
    try {
        ...
        console.error('Error fetching user by userId:', error.response?.data || error.message);
...
const GetSubmittalSpecId = async (projectId, specId, token) => {
    ...
    try {
        ...
        console.error('Error fetching spec details:', error.response?.data || error.message);
        throw error;
    }
};
