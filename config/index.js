...
const config = {
  ...
  APPROVED_EMAILS: parsePairs(process.env.APPROVED_EMAILS, ['email', 'name']).length
    ? parsePairs(process.env.APPROVED_EMAILS, ['email', 'name'])
    : DEFAULT_APPROVED_EMAILS,
  AUTHORIZED_HUBS: parsePairs(process.env.AUTHORIZED_HUBS, ['id', 'name']).length
    ? parsePairs(process.env.AUTHORIZED_HUBS, ['id', 'name'])
    : DEFAULT_AUTHORIZED_HUBS,
  TOKEN_SCOPES: {
    threeLegged: process.env.THREE_LEGGED_SCOPES || 'data:read data:write data:create account:read viewables:read',
    twoLegged: process.env.TWO_LEGGED_SCOPES || 'data:read data:write bucket:create bucket:read bucket:delete'
  },
  ALLOWED_ORIGINS: process.env.ALLOWED_ORIGINS
    ? process.env.ALLOWED_ORIGINS.split(',').map(o => o.trim())
    : [process.env.FRONTEND_URL || 'http://localhost:5173', 'http://localhost:5173', 'https://tad-app-fronend.vercel.app']
};
